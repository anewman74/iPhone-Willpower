...
@implementation DayResultViewController
...
-(void)viewWillAppear:(BOOL)animated {
    ...
    testresultnum  = (int)[[Singleton sharedSingleton] gettestresultnumber];
    //NSLog(@"test result value passed to day result view controller is  %i", testresultnum);
    ...
    if ((int)screensize == 480) {
        ...
        subtitle.text = @"* Total daily scores and average daily rating.";
    ...
    // Create counter of datestrings and load up the correct 7 dates.
    int counter = 0;
    
    // Calculate the first date to show
    int firstDate = (testresultnum - 1) * 7;
    int lastDate = firstDate + 7;
    
    if(sqlite3_prepare_v2(database, [query2 UTF8String],-1, &Statement, nil) == SQLITE_OK){
        while(sqlite3_step(Statement) == SQLITE_ROW){
            dateChosen = (char *)sqlite3_column_text(Statement, 0);
            
            dateCheck = [[NSString alloc] initWithFormat:@"%s",dateChosen];
            
            if ( (counter >= firstDate) && (counter < lastDate) ) {
                //NSLog(@"counter: %i",counter);
                
                // Add date string into an array.
                [dateArray addObject:dateCheck]; 
            }
            counter++;
        }
        sqlite3_finalize(Statement);
    ...
    //NSLog(@"dateArray is  %@", dateArray);
    
    int count = 1;
    
    // Loop through date array
    for (NSString *dateString in dateArray) {
        //NSLog(@"date string in dateArray loop is  %@", dateString);
        
        // Run the database search.
        query3 = [[NSString alloc] initWithFormat: @"SELECT name, scoretitle1, SUM(score1), scoretitle2, SUM(score2),AVG(rating) FROM goals where name = '%@' and datestring = '%@'",nameCheck, dateString];
        
        //NSLog(@"query 3 - %@", query3);
        
        sqlite3_stmt *stmt;    
        if(sqlite3_prepare_v2(database, [query3 UTF8String],-1, &stmt, nil) == SQLITE_OK){
            while(sqlite3_step(stmt) == SQLITE_ROW){
                nameChosen = (char *)sqlite3_column_text(stmt, 0);
                char *scti1 = (char *)sqlite3_column_text(stmt, 1);
                int scor1 = sqlite3_column_int(stmt, 2);
                char *scti2 = (char *)sqlite3_column_text(stmt, 3);
                int scor2 = sqlite3_column_int(stmt, 4);
                int rat = sqlite3_column_int(stmt, 5);
                ...
                scoreLabel1.text = [[[NSString alloc] initWithFormat:@"%@",capitalizedTitle1] autorelease];
                scoreLabel2.text = [[[NSString alloc] initWithFormat:@"%@",capitalizedTitle2] autorelease];
          

                // Add the data to the corresponding row.
                switch (count) {
                    case 1:
                        goalDate1.text = dateString;
                        rating1.text = [[[NSString alloc] initWithFormat:@"%i",rat] autorelease];
                        
                        //If there is no name for label 1.
                        if([capitalizedTitle1 isEqualToString:@"N/A"]){
                            scoreLabel11.text = @"-";
                        }
                        else{
                            scoreLabel11.text = [[[NSString alloc] initWithFormat:@"%i",scor1] autorelease];
                        }
                        
                        //If there is no name for label 2.
                        if([capitalizedTitle2 isEqualToString:@"N/A"]){
                            scoreLabel21.text = @"-";
                        }
                        else{
                            scoreLabel21.text = [[[NSString alloc] initWithFormat:@"%i",scor2] autorelease];
                        }

                        break;
                    case 2:
                        goalDate2.text = dateString;
                        rating2.text = [[[NSString alloc] initWithFormat:@"%i",rat] autorelease];
                        ...
                        if([capitalizedTitle1 isEqualToString:@"N/A"]){
                            ...
                            scoreLabel12.text = [[[NSString alloc] initWithFormat:@"%i",scor1] autorelease];
                        }
                        ...
                        if([capitalizedTitle2 isEqualToString:@"N/A"]){
                            ...
                            scoreLabel22.text = [[[NSString alloc] initWithFormat:@"%i",scor2] autorelease];
                        }
                        break;  
                    case 3:
                        goalDate3.text = dateString;
                        rating3.text = [[[NSString alloc] initWithFormat:@"%i",rat] autorelease];
                        ...
                        if([capitalizedTitle1 isEqualToString:@"N/A"]){
                            ...
                            scoreLabel13.text = [[[NSString alloc] initWithFormat:@"%i",scor1] autorelease];
                        }
                        ...
                        if([capitalizedTitle2 isEqualToString:@"N/A"]){
                            ...
                            scoreLabel23.text = [[[NSString alloc] initWithFormat:@"%i",scor2] autorelease];
                        }
                        break;
                    case 4:
                        goalDate4.text = dateString;
                        rating4.text = [[NSString alloc] initWithFormat:@"%i",rat];
                        ...
                        if([capitalizedTitle1 isEqualToString:@"N/A"]){
                            ...
                            scoreLabel14.text = [[NSString alloc] initWithFormat:@"%i",scor1];
                        }
                        ...
                        if([capitalizedTitle2 isEqualToString:@"N/A"]){
                            ...
                            scoreLabel24.text = [[NSString alloc] initWithFormat:@"%i",scor2];
                        }
                        break;
                    case 5:
                        goalDate5.text = dateString;
                        rating5.text = [[NSString alloc] initWithFormat:@"%i",rat];
                        ...
                        if([capitalizedTitle1 isEqualToString:@"N/A"]){
                            ...
                            scoreLabel15.text = [[NSString alloc] initWithFormat:@"%i",scor1];
                        }
                        ...
                        if([capitalizedTitle2 isEqualToString:@"N/A"]){
                            ...
                            scoreLabel25.text = [[NSString alloc] initWithFormat:@"%i",scor2];
                        }
                        break;
                    case 6:
                        goalDate6.text = dateString;
                        rating6.text = [[NSString alloc] initWithFormat:@"%i",rat];
                        ...
                        if([capitalizedTitle1 isEqualToString:@"N/A"]){
                            ...
                            scoreLabel16.text = [[NSString alloc] initWithFormat:@"%i",scor1];
                        }
                        ...
                        if([capitalizedTitle2 isEqualToString:@"N/A"]){
                            ...
                            scoreLabel26.text = [[NSString alloc] initWithFormat:@"%i",scor2];
                        }
                        break;
                    case 7:
                        goalDate7.text = dateString;
                        rating7.text = [[NSString alloc] initWithFormat:@"%i",rat];
                        ...
                        if([capitalizedTitle1 isEqualToString:@"N/A"]){
                            ...
                            scoreLabel17.text = [[NSString alloc] initWithFormat:@"%i",scor1];
                        }
                        ...
                        if([capitalizedTitle2 isEqualToString:@"N/A"]){
                            ...
                            scoreLabel27.text = [[NSString alloc] initWithFormat:@"%i",scor2];
                        }
                        break;
                    default:
                        break;
...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.title =  NSLocalizedString(@"Daily Comparison", @"daily comparison");	
}
