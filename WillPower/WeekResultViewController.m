...
@implementation WeekResultViewController
...
-(void)viewWillAppear:(BOOL)animated {
    ...
    [formatter3 setDateFormat:@"EEEE, HH, mm, MMM dd yyyy"];
    ...
    if ((int)screensize == 480) {
        ...
        subtitle.text = @"* Total weekly scores and average daily rating.";
    ...
    // Get first Sunday date.
    firstSunDouble  = [[Singleton sharedSingleton] getdoublesun];
    firstSunDate = [NSDate dateWithTimeIntervalSince1970:firstSunDouble];            
    firstSunString = [formatter stringFromDate:firstSunDate];
    
    // Get the last Saturday date.
    lastSatDouble  = [[Singleton sharedSingleton] getdoublesat];
    
    // The double value of one day.
    oneday = 24 * 60 * 60;
    
    // Get the last Sunday date.
    lastSunDouble = lastSatDouble + oneday;
    lastSunDate = [NSDate dateWithTimeIntervalSince1970:lastSunDouble];            
    lastSunString = [formatter stringFromDate:lastSunDate];
    lastSunStringSpecific = [formatter2 stringFromDate:lastSunDate];
    
    // Take into account possible daylight saving.
    if ([lastSunStringSpecific isEqualToString:@"23 00"]) {
        lastSunDouble = lastSunDouble + (60 * 60);
    }
    else if ([lastSunStringSpecific isEqualToString:@"01 00"]) {
        lastSunDouble = lastSunDouble - (60 * 60);
    }
    lastSunDate = [NSDate dateWithTimeIntervalSince1970:lastSunDouble];            
    lastSunString = [formatter stringFromDate:lastSunDate];  
    lastSunStringSpecific = [formatter3 stringFromDate:lastSunDate];
    
    // Get the previous Sunday double date.
    previousSunDouble = lastSunDouble - (oneday * 7);
    previousSunDate = [NSDate dateWithTimeIntervalSince1970:previousSunDouble];            
    previousSunString = [formatter stringFromDate:previousSunDate];
    previousSunStringSpecific = [formatter2 stringFromDate:previousSunDate];
    
    // Take into account possible daylight saving.
    if ([previousSunStringSpecific isEqualToString:@"23 00"]) {
        previousSunDouble = previousSunDouble + (60 * 60);
    }
    else if ([previousSunStringSpecific isEqualToString:@"01 00"]) {
        previousSunDouble = previousSunDouble - (60 * 60);
    }
    previousSunDate = [NSDate dateWithTimeIntervalSince1970:previousSunDouble];            
    previousSunString = [formatter stringFromDate:previousSunDate];
    previousSunStringSpecific = [formatter3 stringFromDate:previousSunDate];
    

    // Count - to measure number of days in week.
    int count = 1;
    
    while (count <= 7) {        
        // Run the database search.
        query2 = [[NSString alloc] initWithFormat: @"SELECT SUM(score1),SUM(score2), AVG(rating) FROM goals where name = '%@' and date >= '%f' and  date < '%f'", nameCheck, previousSunDouble,lastSunDouble];
        ...
        if(sqlite3_prepare_v2(database, [query2 UTF8String],-1, &stmt, nil) == SQLITE_OK){
            while(sqlite3_step(stmt) == SQLITE_ROW){
                ...
                switch (count) {
                    case 1:
                        goalDate1.text = previousSunString;
                        ...
                        break;
                    case 2:
                        goalDate2.text = previousSunString;
                        ...
                        break;  
                    case 3:
                        goalDate3.text = previousSunString;
                        ...
                        break;
                    case 4:
                        goalDate4.text = previousSunString;
                        ...
                        break;
                    case 5:
                        goalDate5.text = previousSunString;
                        ...
                        break;
                    case 6:
                        goalDate6.text = previousSunString;
                        ...
                        break;
                    case 7:
                        goalDate7.text = previousSunString;
        ...
        // Get the following week's previous Sunday and last Sunday
        lastSunDouble = previousSunDouble;
        lastSunDate = [NSDate dateWithTimeIntervalSince1970:lastSunDouble];            
        lastSunString = [formatter stringFromDate:lastSunDate];
        
        previousSunDouble = previousSunDouble - (oneday * 7);
        previousSunDate = [NSDate dateWithTimeIntervalSince1970:previousSunDouble];            
        previousSunString = [formatter stringFromDate:previousSunDate];
        previousSunStringSpecific = [formatter2 stringFromDate:previousSunDate];
...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.title =  NSLocalizedString(@"Weekly Comparison", @"weekly comparison");	
}
...
- (void)dealloc
{
    ...
    [formatter2 release];
    [formatter3 release];
    [firstSunString release];
    [firstSunDate release];
    [lastSunDate release];
    [lastSunString release];
    [lastSunStringSpecific release];
    [previousSunDate release];
    [previousSunString release];
    [previousSunStringSpecific release];
    [app release];
    [goalName release];
    [scoreLabel1 release];
    [scoreLabel2 release];
