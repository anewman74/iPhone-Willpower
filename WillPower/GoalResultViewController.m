...
@implementation GoalResultViewController
@synthesize everyVC;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	for (int i=1; i<count+1; i++) {
		if(row == i)
		{
            ...
            query = [[NSString alloc] initWithFormat: @"SELECT row, date FROM goals where name = '%@' limit 1",nameChosen];
            ...
            if([strSelectedTime isEqualToString:@"December 31 1969"]){
                //NSLog(@"row count in if: %i", datacount);
                ...
                if (self.everyVC == nil)
                {
                    EveryLogViewController *gResultLog = [[EveryLogViewController alloc] initWithNibName: @"EveryLogViewController" bundle:[NSBundle mainBundle]];
                    self.everyVC = gResultLog;
                    [gResultLog release];
                }
                WillPowerAppDelegate *delegate = [[UIApplication sharedApplication] delegate];
                [delegate.navController pushViewController:everyVC animated:YES];
            }
		}
	}
}
