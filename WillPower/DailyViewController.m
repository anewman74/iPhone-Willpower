	...
	for (int i=1; i<count+1; i++) {
		if(row == i)
		{
            ...
            if([strSelectedTime isEqualToString:@"December 31 1969"]){
                ...
                if (self.daylistVC == nil)
                {
                    DayListViewController *aDetail = [[DayListViewController alloc] initWithNibName: @"DayListViewController" bundle:[NSBundle mainBundle]];
                    self.daylistVC = aDetail;
                    [aDetail release];
                }	
                WillPowerAppDelegate *delegate = [[UIApplication sharedApplication] delegate];
                [delegate.navController pushViewController:daylistVC animated:YES];
            }
		}
	}
}
