	...
	for (int i=1; i<count+1; i++) {
		if(row == i)
		{
            ...
            if([strSelectedTime isEqualToString:@"December 31 1969"]){
                ...
                if (self.monthlistVC == nil)
                {
                    MonthListViewController *aDetail = [[MonthListViewController alloc] initWithNibName: @"MonthListViewController" bundle:[NSBundle mainBundle]];
                    self.monthlistVC = aDetail;
                    [aDetail release];
                }	
                WillPowerAppDelegate *delegate = [[UIApplication sharedApplication] delegate];
                [delegate.navController pushViewController:monthlistVC animated:YES];
            }
		}
	}
}
